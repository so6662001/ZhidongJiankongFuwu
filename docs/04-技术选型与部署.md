...
公司技术栈为 **Java**，且明确希望**复用开源 + 二次开发**。在「Java 阵营开源」与「Uptime Kuma」之间对比如下：

| 维度 | Apache HertzBeat（Java） | Uptime Kuma（Node.js） |
| --- | --- | --- |
| 技术栈 | **Java + Spring Boot**（前端 Angular/TS）| Node.js + Vue |
| 与公司栈契合 / 二开 | ✅ 一致，可直接维护/改源码 | ❌ 内核为 JS，二开需切栈，仅能外挂集成层 |
| API 接口监控 | ✅ HTTP + JsonPath、多步骤鉴权(先取 token)、SSL 证书、阈值表达式 | ✅ HTTP/Keyword/JSON Query，较简单 |
| 邮件 + 企业微信 | ✅ 原生，且支持按标签/级别路由到接收人 | ✅ 原生 WeCom + 邮件（渠道多，偏国外）|
| 告警治理 | ✅ 阈值规则 + 分组/收敛/静默/抑制 | ⚠️ 仅重试，治理弱 |
| 数据库管理 | ✅ 元数据库(MySQL/PG)+时序库(VictoriaMetrics/IoTDB 等) | ⚠️ 内置库，模型简单 |
| 编程化批量纳管 | ✅ 完整 REST API | ❌ 写操作走 Socket.IO，无稳定写 API |
| 规模/扩展 | ✅ 多采集器集群、云边协同，适合平台级 | ⚠️ 单体，大规模吃力 |
| 上手速度 | `docker run -p 1157:1157 apache/hertzbeat` | 同样轻量，UI 更快上手 |
| 协议 | Apache 2.0 | MIT |

**最终决策：采用 Apache HertzBeat 二次开发。** 理由：技术栈一致、二开友好、API 监控+企业微信/邮件+告警治理+数据库开箱即得、REST API 便于批量纳管、可集群扩展，契合软件公司「平台级 + 多接口 + 要数据库管理 + 要定制」的诉求。Uptime Kuma 更适合「快速、轻量、小规模、几乎不改代码」的 PoC 场景。

> MVP 落地方式与分阶段 AI 开发提示词见 `06-MVP开发提示词.md`：HertzBeat 以官方镜像部署（不改源码），旁挂一个 Java(Spring Boot) 集成层，通过 HertzBeat REST API（批量纳管）与告警 Webhook（告警归档）对接。
